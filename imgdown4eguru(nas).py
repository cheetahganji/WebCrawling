    ...
    if path and filename and content:
        ...
        if not os.path.exists(path + '/' + filename + '.txt'):
            with open(path + '/' + filename + '.txt', 'w') as f:
                for c in content:
                    f.writelines(c[0].encode('utf-8') + '\t' + c[1].encode('utf-8') + '\n')
...
def save_img(page_info):
    ...
    full_path = path + '/'.encode('utf-8') + page_nm
...
if __name__ == "__main__":
    ...
    path = "/volume1/photo/comicbooks/" + comicbook_nm
    ...
    for v in vol_info:
        ...
        if not os.path.exists(path.encode('utf-8') + '/'.encode('utf-8') + vol_nm.encode('utf-8')):
	        ...
	        for i in img_tag_list:
	            ...
	            page_info.append([page_img_nm.encode('utf-8'), page_img, path.encode('utf-8') + '/'.encode('utf-8') + vol_nm.encode('utf-8')])
	
	        
	        if not os.path.exists(path.encode('utf-8') + '/'.encode('utf-8') + vol_nm.encode('utf-8')):
	            os.mkdir(path.encode('utf-8') + '/'.encode('utf-8') + vol_nm.encode('utf-8'))
	
	        print(vol_nm.encode('utf-8'), 'start!')
	        pool.map(save_img, page_info)
	        print(vol_nm.encode('utf-8'), 'end!\n')
