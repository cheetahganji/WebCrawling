...
"""
Synology NAS에서 실행시키기 위해 기존 소스를 변환함(유니코드,UTF8 등의 문제로 로컬에서 잘 작동하는게 NAS에선 작동하지 않았음)
...
def save_img(page_info):
    ...
    dst_path = base_path + book_no + '/'.encode('utf-8') + img_name.encode('utf-8')
...
if __name__ == "__main__":
    ...
    book_name = '슬램덩크'
    base_path = '/volume1/photo/comicbooks/'.encode('utf-8') + book_name.encode('utf-8') + '/'.encode('utf-8')
    
    book_info = get_book_info(web_url, tag_nm, book_name)
    

    for bi in book_info:
        bi[0] = bi[0].encode('utf-8')

        if bi[0] == '슬램덩크 12권'.encode('utf-8'):    # 권 수가 많은 경우 van될까봐 1권 씩...
